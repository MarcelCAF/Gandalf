...
CONFIG = {
    # OrcaScan
    "ORCA_URL": "https://cloud.orcascan.com/sheets/DEIN_SHEET_ID_HIER",
    ...
    "ABHOLER_DB_SHEET_ID": "",   # wird aus config_geheim.py geladen

}

# Geheime Zugangsdaten aus config_geheim.py laden (nicht im Git-Repo)
try:
    import config_geheim as _cfg
    CONFIG["ORCA_API_KEY"]        = _cfg.ORCA_API_KEY
    CONFIG["ABHOLER_DB_SHEET_ID"] = _cfg.ABHOLER_DB_SHEET_ID
    CONFIG["ORCA_URL"]            = f"https://cloud.orcascan.com/sheets/{_cfg.ORCA_TAGESBOTE_SHEET_ID}"
except ImportError:
    pass  # Ohne config_geheim.py laufen (Keys muessen dann manuell in CONFIG eingetragen sein)
...
def map_zahlung_wawican(vor_ort_wert, zahlungsstatus_wert="") -> str:
    """
    Bestimmt den Zahlungsstatus für WAWICAN-Pakete.
    Vor Ort wenn:
      - vor Ort Zahlung = "Ja"
      - ODER Zahlungsstatus ist leer          (WAWICAN-Bug)
      - ODER Zahlungsstatus = "Rechnung versendet"
    """
    vor_ort = "" if pd.isna(vor_ort_wert) else str(vor_ort_wert).strip().lower()
    zs      = "" if pd.isna(zahlungsstatus_wert) else str(zahlungsstatus_wert).strip().lower()

    if vor_ort == "ja":
        return "Vor Ort"
    if not zs or zs == "rechnung versendet":
        return "Vor Ort"
    if vor_ort == "nein":
        return "Erhalten"
    return ""
...
def build_orca_from_wawican(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Zahlungsstatus optional (fehlt in älteren WAWICAN-Exporten)
    if "Zahlungsstatus" in df.columns:
        out["Zahlung"] = [map_zahlung_wawican(v, z) for v, z in
                          zip(df["vor Ort Zahlung"], df["Zahlungsstatus"])]
    else:
...
def upload_merged_file_via_cdp(merged_xlsx: Path, log_file: Path, merged_df=None) -> None:
    ...
    with sync_playwright() as p:
        ...
        # Etiketten drucken: ZPL-Direktdruck oder alter Sumatra-Weg
        if CONFIG.get("LABEL_VIA_ZPL"):
            print_labels_zpl(merged_df, log_file)
        else:
...
def _build_symbol_zpl(kiosk: str, zahlung: str) -> str:
    """
    Erzeugt ZPL-Code für Symbole auf dem Etikett (zentriert).
    Kiosk Kissel       -> Kreis ausgefüllt
    Kiosk Mörfelder    -> Ring
    Zahlung Vor Ort    -> Ausrufezeichen im Rechteck
    Beide gleichzeitig -> Kiosk oben, Zahlung unten
    """
    k = kiosk.strip().lower()
    z = zahlung.strip().lower()

    has_kiosk   = k in ("kissel", "kiosk mörfelder")
    has_zahlung = z == "vor ort"

    if not has_kiosk and not has_zahlung:
        return ""

    # Symbole übereinander: Kiosk Y=81, Zahlung Y=191 (1cm nach links verschoben)
    if has_kiosk and has_zahlung:
        kiosk_sym = ("^FO352,81^GE80,80,80,B^FS" if k == "kissel"
                     else "^FO352,81^GE80,80,7,B^FS")
        zahlung_sym = ("^FO352,191^GB80,80,5^FS"
                       "^FO384,209^GB12,32,12^FS"
                       "^FO384,254^GB12,12,12^FS")
        return kiosk_sym + zahlung_sym

    # Nur Kiosk (zentriert Y=136)
    if has_kiosk:
        return ("^FO352,136^GE80,80,80,B^FS" if k == "kissel"
                else "^FO352,136^GE80,80,7,B^FS")

    # Nur Zahlung (zentriert Y=136)
    return ("^FO352,136^GB80,80,5^FS"
            "^FO384,154^GB12,32,12^FS"
            "^FO384,199^GB12,12,12^FS")


def _name_font_size(name: str, has_symbols: bool = False) -> int:
    """
    Ermittelt die Schriftgröße je nach Namenslänge und ob Symbole vorhanden sind.
    Mit Symbolen ist weniger Platz, daher strengere Schwellwerte.
    """
    n = len(name)
    if has_symbols:
        # Platz reicht nur bis zur Symbol-Spalte (~317 dots)
        if n <= 8:  return 55
        if n <= 12: return 42
        if n <= 16: return 35
        return 28
    else:
        # Platz bis zum QR-Code (~420 dots)
        if n <= 15: return 55
        if n <= 22: return 42
        return 35


def _build_label_zpl(name: str, barcode: str, kiosk: str, zahlung: str = "") -> str:
    """Erzeugt ZPL-Code für ein Etikett (102x38mm, 203dpi)."""
    symbol = _build_symbol_zpl(kiosk, zahlung)
    fs = _name_font_size(name, has_symbols=bool(symbol))
    ...
    return (
        ...
        "^PW816"
        "^LL304"
        f"^FO25,54^A0N,{fs},{fs}^FD{name}^FS"
        f"^FO25,134^A0N,32,32^FD{barcode}^FS"
        f"^FO25,210^A0N,24,24^FD{datum}^FS"
        f"^FO25,279^A0N,26,26^FD{kiosk}^FS"
        f"^FO456,39^BQN,2,9^FDQA,{barcode}^FS"
        f"{symbol}"
        "^XZ"
    )


def print_labels_zpl(orca_df, log_file: Path) -> None:
    """Druckt Etiketten per ZPL direkt an den Zebra-Drucker (ohne SumatraPDF/OrcaScan)."""
    try:
        import win32print
    except ImportError:
        log_write(log_file, "[ZPL] pywin32 nicht installiert – Druck übersprungen.", level="WARN")
        return

    printer_name = str(CONFIG.get("LABEL_ZPL_PRINTER", ""))
    if not printer_name:
    ...
    for _, row in orca_df.iterrows():
        ...
        if not barcode:
            continue
        zpl = _build_label_zpl(name, barcode, kiosk, zahlung)
        try:
            h = win32print.OpenPrinter(printer_name)
            try:
                win32print.StartDocPrinter(h, 1, ("Gandalf ZPL", None, "RAW"))
            ...
            log_write(log_file, f"  [ZPL] Etikett gedruckt: {barcode}", level="OK")
            printed += 1
        except Exception as e:
            log_write(log_file, f"  [ZPL] Fehler bei {barcode}: {e}", level="WARN")
            failed += 1

    level = "OK" if failed == 0 else "WARN"
    log_write(log_file, f"[ZPL] Fertig: {printed} gedruckt | {failed} Fehler", level=level)
    ...
    def run_pipeline():
        ...
        try:
            ...
            if merged_file is None:
                ...
                upload_merged_file_via_cdp(merged_file, log_file, merged_df)
                try:
