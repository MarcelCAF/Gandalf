...
CONFIG = {
    ...
    "BASE_PICKUP_DIR": Path(r"C:\Users\Abfuellung 15\Documents\Marcels Skripts\Gandalf\PICKUP"),
...
def build_orca_from_caf(df: pd.DataFrame) -> pd.DataFrame:
    required = ["Best.-Nr.", "Datum", "Name", "Bestellwert", "Lieferung", "Zahlung", "Status", "Versicher."]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"[CAF] Fehlende Spalten: {missing}")

    if CONFIG["CAF_EXPORT_ONLY_PICKUP"]:
        pickup_value = str(CONFIG["CAF_PICKUP_VALUE"]).strip().lower()
        mask = df["Lieferung"].astype(str).str.strip().str.lower() == pickup_value
        df = df[mask].copy()

    sort_series = pd.to_datetime(df["Datum"].astype(str).str.strip(), format="%d.%m.%Y %H:%M", errors="coerce")
    df = df.assign(_sort_date=sort_series).sort_values("_sort_date", ascending=True).drop(columns="_sort_date")
...
def upload_merged_file_via_cdp(merged_xlsx: Path, log_file: Path) -> None:
    ...
    with sync_playwright() as p:
        ...
        log_write(log_file, "Import abgeschlossen. Chrome/Orca bleibt offen (detached).")
