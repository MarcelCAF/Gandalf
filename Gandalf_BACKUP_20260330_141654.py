...
def build_orca_from_wawican(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[~df["Id"].astype(str).str.strip().str.startswith("http")].copy()
...
def process_pickup_queue_merge(base_dir: Path, log_file: Path) -> tuple[Path | None, int, int, int]:
    ...
    if not all_parts:
        ...
        return None, processed, failed, 0
    ...
    return out_path, processed, failed, len(merged)
...
def _label_select_new_rows(page, log_file: Path, rows_before: int, rows_after: int) -> None:
    """
    Wählt nur die neu importierten Zeilen aus.
    Klickt per Playwright auf Position (5,5) der th[data-row-header] –
    oben links = Zeilennummer, unterhalb liegt der 'View'-Button.
    Fällt auf Alle-Auswählen zurück bei Fehler oder 0 Treffern.
    """
    n_new = rows_after - rows_before
    if n_new <= 0:
        log_write(log_file, "  WARNUNG: Keine neuen Zeilen erkannt, wähle alle aus.", level="WARN")
        _label_select_all_rows(page, log_file)
        return
    ...
    try:
        for i in range(rows_before, rows_after):
            ...
            if th.count() == 0:
                log_write(log_file, f"  WARNUNG: tr[data-index='{i}'] nicht gefunden.", level="WARN")
                continue
            # Ins Sichtfeld scrollen und warten bis das Grid neu gerendert hat
            th.scroll_into_view_if_needed(timeout=3000)
            page.wait_for_timeout(150)
            # Locator nach Scroll neu holen (virtuelles Grid rendert Zeilen neu)
            th = page.locator(f'tr[data-index="{i}"] th[data-row-header="true"]')
            # Erste Zeile: normaler Klick (startet Auswahl)
            # Weitere Zeilen: Ctrl+Klick (fügt zur Auswahl hinzu, kein Deselektieren)
            if selected == 0:
        ...
        page.wait_for_timeout(300)
        log_write(log_file, f"  {selected} Zeilen ausgewählt.")
        if selected == 0:
            log_write(log_file, "  WARNUNG: 0 Zeilen ausgewählt, wähle alle aus.", level="WARN")
            _label_select_all_rows(page, log_file)

    except Exception as e:
        log_write(log_file, f"  WARNUNG: Zeilenauswahl fehlgeschlagen ({e}), wähle alle aus.", level="WARN")
        _label_select_all_rows(page, log_file)
...
def _label_print_sumatra(pdf_bytes: bytes, log_file: Path) -> None:
    ...
    try:
        result = subprocess.run(
            [sumatra, "-print-to", printer, "-print-settings", "fit,landscape", str(tmp_path)],
            timeout=30,
            capture_output=True,
            text=True,
        )
...
def print_labels(page, log_file: Path, rows_before: int = 0, rows_after: int = 0) -> None:
    ...
    if not sumatra or not printer:
        ...
        return
    try:
        _label_select_new_rows(page, log_file, rows_before=rows_before, rows_after=rows_after)
...
def main():
    ...
    def run_pipeline():
        ...
        try:
            ...
            merged_file, processed, failed, total_rows = process_pickup_queue_merge(base_dir, log_file)
